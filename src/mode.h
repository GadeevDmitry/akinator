...
MODE(DOWNLOAD   , 2, {mode_download  ();        }, "download base from file"                        )
MODE(EXIT       , 3, {save_data      (ROOT);
                      return              0;    }, "stop akinator and exit"                         )
MODE(DUMP       , 4, {Tree_dump      (ROOT);    }, "dump current tree in log-file"                  )
MODE(DEFINITION , 5, {mode_definition();        }, "show definition of concept from the base"       )
MODE(COMPARE    , 6, {mode_compare   ();        }, "show comparing between two concepts from base"  )
